    ...
    return (
        <Box>
            <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", mb: 2 }}>
                <Typography variant="h4" sx={{ fontSize: "30px", fontWeight: 500, color: "#0f172a" }}>
                    {post.title ?? "(제목 없음)"}
                </Typography>
                {isPostOwner && (
                    <Stack direction="row" spacing={1}>
                        <Button variant="text" sx={{ fontSize: "20px", color: "#64748b" }} onClick={onEdit}>수정</Button>
                        <Button variant="text" sx={{ fontSize: "20px", color: "#ef4444" }} onClick={onDelete}>삭제</Button>
                    </Stack>
                )}
            </Box>
            ...
            <Stack direction="row" spacing={2} alignItems="center" sx={{ mb: 5 }}>
                ...
                <Box>
                    <Typography sx={{ fontWeight: 600, fontSize: "16px", color: "#334155" }}>
                        {post.author?.nickname ?? post.writer ?? "익명"}
                    </Typography>
                    <Stack direction="row" spacing={1} alignItems="center">
                        <Typography sx={{ color: "#94a3b8", fontSize: "15px" }}>{rgstDate}</Typography>
                        {isEdited && (
                            <Typography sx={{ color: "#3b82f6", fontSize: "15px", fontWeight: 500 }}>
                                (수정 {updtDate})
                            </Typography>
                        )}
